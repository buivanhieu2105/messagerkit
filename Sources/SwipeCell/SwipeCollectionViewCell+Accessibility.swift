...
extension SwipeCollectionViewCell {
    ...
    open override var accessibilityCustomActions: [UIAccessibilityCustomAction]? {
        get {
            ...
            let leftActions = kDelegate?.collectionView(collectionView, editActionsForItemAt: indexPath, for: .left) ?? []
            let rightActions = kDelegate?.collectionView(collectionView, editActionsForItemAt: indexPath, for: .right) ?? []
