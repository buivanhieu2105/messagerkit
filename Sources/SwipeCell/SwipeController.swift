...
class SwipeController: NSObject {
    ...
    weak var swipeable: (SwipeCollectionViewCell & Swipeable)?
    ...
    init(swipeable: SwipeCollectionViewCell & Swipeable, actionsContainerView: UIView) {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .ended, .cancelled, .failed:
            ...
            if actionsView.expanded == true, let expandedAction = actionsView.expandableAction  {
                ...
                let targetOffset = targetCenter(active: swipeable.state.isActive, targetCenter: target.center)
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        guard let cell = self.swipeable as? MessageCollectionViewCell else { return }
...
//        }
        ...
        // custom code for layout actions view with MessageKit
        var top: CGFloat = 0
        cell.contentView.subviews.forEach { (view) in
            if view is MessageContainerView {
        ...
        cell.contentView.subviews.forEach { (view) in
            if view is AvatarView {
                if view.isHidden {
                    // It's the case avatar is hidden
                    cell.contentView.subviews.forEach { (sview) in
                        if sview is MessageContainerView {
                            actionsView.bottomAnchor.constraint(equalTo: sview.bottomAnchor).isActive = true
                            return
                        }
                    }
                } else {
        ...
        //actionsView.heightAnchor.constraint(equalTo: cell.heightAnchor).isActive = true
        actionsView.widthAnchor.constraint(equalTo: cell.widthAnchor, multiplier: 2).isActive = true
        actionsView.topAnchor.constraint(equalTo: cell.topAnchor, constant: top).isActive = true
        if orientation == .left {
            actionsView.rightAnchor.constraint(equalTo: actionsContainerView.leftAnchor).isActive = true
        } else {
            actionsView.leftAnchor.constraint(equalTo: actionsContainerView.rightAnchor).isActive = true
    ...
    func targetCenter(active: Bool, targetCenter: CGPoint? = nil) -> CGFloat {
        ...
        if let targetCenter = targetCenter {
            let distance = swipeable.bounds.midX - targetCenter.x
            if distance < actionsView.preferredWidth / 2 {
...
extension SwipeController: UIGestureRecognizerDelegate {
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer == panGestureRecognizer,
            ...
            if self.swipeable?.state == SwipeState.right {
